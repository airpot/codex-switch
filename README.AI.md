...
The headless localhost worker provides strict provider priority, pre-response failover, circuit breakers, persistent local authentication across normal restarts, and activation backup restoration. It preserves the active Codex `model_provider` id so existing session history remains in the same namespace. Provider selection is not sticky across requests: the first configured provider remains primary, while later providers are fallbacks only during the current request or while an earlier circuit is cooling. Responses routing classifies typed SSE failures and HTTP 200 JSON semantic failures; lifecycle events, comments, heartbeats, empty events, and `[DONE]` do not commit a response by themselves. Upstream `Retry-After` controls 429 cooldown, and cooling circuits automatically enter a half-open probe without a reset command. Strict relay normalization preserves client-provided `prompt_cache_key` and `prompt_cache_retention`, then canonicalizes JSON object-key order while preserving array order. Cache trace logs use hashes rather than request content or cache-key values. Attempt logs include the actual provider, primary/fallback role, upstream HTTP status, sanitized upstream request id, and token/cache usage when supplied by the upstream.
